...
// 		CamParam.readFromXMLFile(cml("-c"));
	...
	TheMarkerMapConfig.readFromFile("map.yml");
	string dictStr = TheMarkerMapConfig.getDictionary();
	MDetector.setDictionary(dictStr); // ARUCO_MIP_36h12
	...
	while (1)
	{
		...
		for (auto idx : TheMarkerMapConfig.getIndices(Markers))
